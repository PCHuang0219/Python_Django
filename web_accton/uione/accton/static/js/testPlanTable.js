...
let getTestPlanTable = function(){
    ...
    $.ajax({
        type: "get",
        url: "http://"+ Config.ip_address + Config.port + "/test/testPlanManagement/testPlanTable/",
        data:{condition},
        dataType: "json",
        success: function(data, status){
            ...
            for(var i=0,x=data.length-1 ; x >= 0 ; x--,i++)
            {
                ...
                "<td data-th='Detail'>" + "<button class='seeMore' type='button'>Detail</button>"+"</td>" +
                "<td data-th='Execute'>" + "<button class='execute' type='button'>Run</button>"+"</td>" +
                "</tr>"
            }
            $(".table tbody").append(html);
            $(".seeMore").click(function(){
                localStorage.setItem("test_plan_id",$(this).parent().parent().children().first().next().attr("test_plan_id"));
                location.href = "http://" + Config.ip_address + Config.port + "/test/testPlanManagement/testPlanDetail"
            });
            $(".execute").click(function(){
                test_plan_id = $(this).parent().parent().children().first().next().attr("test_plan_id");
                location.href = "http://" + Config.ip_address + Config.port + "/test/?test_plan_id=" + test_plan_id
            });
            $(".edit").click(function(){
                localStorage.setItem("test_plan_id",$(this).parent().parent().children().first().next().attr("test_plan_id"));
                location.href = "http://" + Config.ip_address + Config.port + "/test/modifyTestPlan"
            });
        },
    })
    localStorage.clear();
