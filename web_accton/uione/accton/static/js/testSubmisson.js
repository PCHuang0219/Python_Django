...
let saveInformation = function(){
    ...
    location.href = "http://" + Config.ip_address + Config.port + "/test/jenkinsViews/" + url
}
...
let submitTestCase = function(){
    $('#submitTestCase').click(function(){
        if ($(this).attr('id') != 'disableButton'){
            ...
            var radios = document.getElementsByName('time_period');
            for (var i = 0, length = radios.length; i < length; i++){
                if (radios[i].checked){
                    if (radios[i].value == '1'){   
                        hours = document.getElementById("time_period_time").value;
                        if (hours == ''){
                            alert("Please enter the hours you want to run.")
                        }
                        else{
                            time_period = String(hours) + ' hours'
                        }
                    }
                    else if (radios[i].value == '0'){ 
                        times = document.getElementById("run_times").value;
                        if (times == ''){
                            alert("Please enter the times you want to run.")
                        }
                        else{
                            time_period = String(times) + ' times'
                        }
                    }
                }
            }
...
let showDetailConfig = function(){
    $(".see_detail").click(function(){
        if($(this)[0].checked){
            ...
            html = '<h1 class="modal-title">' + title +'<button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button></h1>'
            $('.modal-header').append(html)
            $('#example_image').empty()
            html = '<img src="/static/images/FB/' + title + '.png" style="width:100%">'
            $('#example_image').append(html)
        }
    })
}
