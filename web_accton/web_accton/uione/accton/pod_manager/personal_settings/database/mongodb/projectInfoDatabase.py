...
class Project_Info_Database():
    ...
    def insert_issue(self,issue_info_data):
        model_list_cursor = self.main_project_database.find({"project_name":issue_info_data["project"]},{"_id":0,"cust_model":1})
        model = ''
        for model_org in model_list_cursor:
            data = model_org["cust_model"]
            model += data + " "
        issue_info_data["model"]=model
