...
def submit_testcase(request):
    ...
    time_period = data_list['time_period'][0]
    thread_list = []
    for i in range(int(thread_num)):
        key = 'testList[' + str(i) + '][]'
        thread_list.append(data_list[key])

    job_info = Job_Information(project,job_name,job_description,job_platform,job_model,user_id,job_topo,image_version,thread_list, time_period)
