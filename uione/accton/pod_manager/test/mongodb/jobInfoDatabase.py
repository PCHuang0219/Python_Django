...
class Job_Info_Database():
    ...
    def get_list_by_condition(self,condition):
        data_list = []
        condition = json.loads(condition)
        data_list_cursor = self._database.find(condition,{"_id":0}).sort([("submit_time",1)])
        for data in data_list_cursor:
            data_list.append(JSONEncoder().encode(data))
        return data_list
    ...
    def update_test_report_url_by_id(self, job_id, path):
        data = self._database.update({"_id":ObjectId(job_id)}, {'$set':{'test_report_url':path}})
    ...
    def get_selection_by_condition(self,condition):
        data_list = []
        buff_list = []
        data_list_cursor = self._database.find({},{"_id":0,condition:1})
        for data in data_list_cursor:
            data = data[condition]
...
class Test_Plan_Info_Database():
    def __init__(self):
        self._database = database.get_collection("test_plan_info")
    
    def insert(self,data_info):
        data_id = self._database.insert_one(data_info.to_json()).inserted_id
        return data_id
    
    def update(self, data_info, _id):
        data_id = self._database.update({'_id':ObjectId(_id)},{'$set':data_info.to_json()})
        return data_id
    ...
    def get_tests_data_by_test_plan_id(self,test_plan_id):
        job_data_cursor = self._database.find({"_id":ObjectId(test_plan_id)},{"_id":0})
        return {"data":job_data_cursor[0]["testcase_list"],"detail":job_data_cursor[0]}
